...
import co.edu.sena.gestion_turistica.service.TransporteService;
...
@PostMapping()
public ServerResponseAll create(@RequestBody TransporteDto request){
...
public ServerResponseAll ListAll(){
    List<TransporteDto> dtos = this.service.getAll();
...
public ServerResponseAll getById(@PathVariable("id") Long id){
    ...
    TransporteDto dto = this.service.getById(id);
    return ServerResponseAll.builder()
...
public ServerResponseAll update(@PathVariable("id") Long id, @RequestBody TransporteDto request) {
request.setId(id);
